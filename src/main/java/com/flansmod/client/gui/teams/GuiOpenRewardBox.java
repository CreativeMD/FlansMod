...
public class GuiOpenRewardBox extends GuiTeamsBase 
{
			...
			case SLOWING_DOWN:
			{
				...
				if(spinSpeed <= -Acceleration())
				{
					...
					FMLClientHandler.instance().getClient().getSoundHandler().playSound(
							new PositionedSoundRecord(FlansModResourceHandler.getSound("UnlockNotch"), SoundCategory.PLAYERS, 1.0F, 2.0f, 
									allowUserInput, target, null, (float)mc.player.posX, (float)mc.player.posY, (float)mc.player.posZ));
	...
	public void drawScreen(int i, int j, float f)
	{
		...
		{
			FMLClientHandler.instance().getClient().getSoundHandler().playSound(
					new PositionedSoundRecord(FlansModResourceHandler.getSound("UnlockNotch"), SoundCategory.PLAYERS, 0.5F, 1.0f, 
							allowUserInput, postIndex, null, (float)mc.player.posX, (float)mc.player.posY, (float)mc.player.posZ));
