...
public class EntityAAGun extends Entity implements IEntityAdditionalSpawnData
{
    ...
    public void setPositionAndRotationDirect(double x, double y, double z, float yaw, float pitch, int posRotationIncrements, boolean teleport)
    {
		...
		sYaw = yaw;
		sPitch = pitch;
		sUpdateTime = posRotationIncrements;
	}
	...
	public boolean attackEntityFrom(DamageSource damagesource, float i)
	{
		if (damagesource.damageType.equals("player"))
		{
			...
			{
				return getControllingPassenger().attackEntityFrom(damagesource, i);
	...
	public boolean processInitialInteract(EntityPlayer player, EnumHand hand) //interact : change back when Forge updates
	{
		...
		// Mount gun
		if (getControllingPassenger() != null && (getControllingPassenger() instanceof EntityPlayer) && getControllingPassenger() != player)
		{
			return true;
		}
		if (!world.isRemote)
		{
			...
			for (int i = 0; i < (type.shareAmmo ? 1 : type.numBarrels); i++)
			{
				if (ammo[i] == null)
				{
					...
					if (slot >= 0)
					{
						ammo[i] = player.inventory.getStackInSlot(slot).copy();
						ammo[i].setCount(1);
