...
public class InventoryHelper 
{
	...
	public static int storeItemStack(IInventory inventory, ItemStack stack)
	{
		for (int i = 0; i < inventory.getSizeInventory(); ++i)
		{
			...
			if (oldStack.getItem() == stack.getItem() && oldStack.isStackable() && oldStack.getCount() < oldStack.getMaxStackSize() && oldStack.getCount() < inventory.getInventoryStackLimit() && (!oldStack.getHasSubtypes() || oldStack.getItemDamage() == stack.getItemDamage()) && ItemStack.areItemStackTagsEqual(oldStack, stack))
			{
				return i;
			}
		}
