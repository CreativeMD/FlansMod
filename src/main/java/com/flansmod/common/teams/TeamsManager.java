...
public class TeamsManager
{
	...
	{
		if( event.getEntityPlayer().inventory.getCurrentItem().getItem() instanceof ItemOpStick)
	...
	public void onEntityHurt(LivingAttackEvent event) 
	{
		...
		if(event.getEntity() instanceof EntityPlayerMP)
		{
			...
			if(source instanceof EntityDamageSource && source.getTrueSource() instanceof EntityPlayerMP)
			{
	...
	public void playerInteracted(LeftClickBlock event)
	{
		if(!enabled)
			return;
		if(event.getEntityPlayer().getHeldItemMainhand().getItem() instanceof ItemGun)
		{
			event.setCanceled(true);
			return;	
		}
	...
	public void playerDrops(PlayerDropsEvent event)
	{
		...
		if(weaponDrops == 2)
		{
			for(EntityItem entity : event.getDrops())
			{
				...
				{	
					...
					gunEntity.setInfinitePickupDelay();
		...
		for(EntityItem entity : dropsToThrow)
		{
			...
			for(EntityItem ammoEntity : event.getDrops())
			{
				...
				if(ammoItemstack.getItem() instanceof ItemShootable)
				{
	...
	public void sendTeamsMenuToPlayer(EntityPlayerMP player, boolean info)
	{
		...
		boolean playerIsOp = player.getServer().getPlayerList().canSendCommands(player.getGameProfile());
	...
	public void resetInventory(EntityPlayer player)
	{
		...
		//Set team armour
		if(team.hat != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.HEAD.getSlotIndex(), team.hat.copy());
		if(team.chest != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.CHEST.getSlotIndex(), team.chest.copy());
		if(team.legs != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.LEGS.getSlotIndex(), team.legs.copy());
		if(team.shoes != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.FEET.getSlotIndex(), team.shoes.copy());
		...
		if(playerClass.GetHat() != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.HEAD.getSlotIndex(), playerClass.GetHat().copy());
		if(playerClass.GetChest() != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.CHEST.getSlotIndex(), playerClass.GetChest().copy());
		if(playerClass.GetLegs() != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.LEGS.getSlotIndex(), playerClass.GetLegs().copy());
		if(playerClass.GetShoes() != null)
			player.inventory.armorInventory.set(EntityEquipmentSlot.FEET.getSlotIndex(), playerClass.GetShoes().copy());
