...
public abstract class EntityDriveable extends Entity implements IControllable, IExplodeable, IEntityAdditionalSpawnData
{
	...
	{
		return seats != null && seats[0] != null && seats[0].getControllingEntity() instanceof EntityPlayer && ((EntityPlayer)seats[0].getControllingPassenger()).capabilities.isCreativeMode;
	}
    ...
    public void onUpdate()
    {
  			...
  			for(float x = box.x; x <= box.x + box.w; x++)
  			{
  				for(float y = box.y; y <= box.y + box.h; y++)
  				{
  					for(float z = box.z; z <= box.z + box.d; z++)
  					{
						...
						if(!cancelled)
						{
	  						if(type.materialsHarvested.contains(state.getMaterial()) && state.getBlockHardness(world, pos) >= 0F)
	  						{
		...
		if(getControllingPassenger() != null && isDead)
		{
