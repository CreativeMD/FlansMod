...
public class EntityVehicle extends EntityDriveable implements IExplodeable
{
		...
		for(int i = 0; i <= type.numPassengers; i++)
		{
			if(seats[i].processInitialInteract(player, hand))
			{
				if(i == 0)
				{
					...
					FlansMod.proxy.doTutorialStuff(player, this);
				}
				return true;
			}
		}
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
		...
		switch(key)
		{
			...
			case 6 : //Exit : Get out
			{
				seats[0].getControllingEntity().startRiding(null);
		  		return true;
			}
			case 7 : //Inventory
			{
	...
	public void onUpdate()
	{
		...
		for(EntityWheel wheel : wheels)
		{
			...
			boolean canThrustCreatively = !TeamsManager.vehiclesNeedFuel || (seats != null && seats[0] != null && seats[0].getControllingPassenger() instanceof EntityPlayer && ((EntityPlayer)seats[0].getControllingPassenger()).capabilities.isCreativeMode);
