...
public class EntityMecha extends EntityDriveable
{
	...
	public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
	{
		...
		{
			if(seats[i].processInitialInteract(player, hand))
				return true;
		}
		return false;
	}
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
		...
		switch(key)
		{
			...
			case 6 : //Exit : Get out
			{
				seats[0].getControllingPassenger().startRiding(null);
		  		return true;
			}
			case 7 : //Inventory
			{
		...
		if(left? isPartIntact(EnumDriveablePart.leftArm) : isPartIntact(EnumDriveablePart.rightArm))
		{
			...
			else if(heldItem instanceof ItemGun)
			{
				...
				if(delay <= 0)
				{
					...
					if(bulletStack == null)
					{
						 gunItem.Reload(heldStack, world, this, driveableData, left, true, true, (infiniteAmmo() || creative()));
					}
							...
							if(!cancelled)
							{
								...
								if(vacuumItems)
								{
									for(ItemStack stack : blockHit.getDrops(world, new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z), state, 0))
									{
										...
										if(fuelCheck && wasteCompact() && stack.getItem() instanceof ItemBlock && (((ItemBlock)stack.getItem()).getBlock() == Blocks.COBBLESTONE || ((ItemBlock)stack.getItem()).getBlock() == Blocks.DIRT || ((ItemBlock)stack.getItem()).getBlock() == Blocks.SAND))
										{
											stack.setCount(0);
											if (!((EntityPlayer)seats[0].getControllingPassenger()).capabilities.isCreativeMode)
