...
public class EntityParachute extends Entity implements IEntityAdditionalSpawnData
{
	...
	public void onUpdate()
	{
		...
		{
			EntityLivingBase living = (EntityLivingBase) getControllingPassenger();
			float speedMultiplier = 0.002F;
			double moveForwards = living.moveForward;
			double moveStrafing = living.moveStrafing;
			double sinYaw = -Math.sin((living.rotationYaw * (float)Math.PI / 180.0F));
			double cosYaw = Math.cos((living.rotationYaw * (float)Math.PI / 180.0F));
